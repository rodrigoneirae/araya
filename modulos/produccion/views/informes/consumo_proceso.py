...
class IndexInformeConsumoProcesoView(LoginRequiredMixin, TemplateView):
    template_name = 'modulos/produccion/informes/consumo_proceso.html'
    ...
    def _get_datos(self, fecha_inicio: str, fecha_fin: str, proceso_cod: str | None):
        ...
        qs = Movs.objects.select_related("codigo", "tipo").filter(tipo__cod__in=[10], linea__gt=0)
        ...
        for m in qs.iterator():
            ...
            numero_ot = int(m.docref) if m.docref else 0
    ...
    def _generar_excel(self, request: HttpRequest) -> HttpResponse:
        ...
        def agregar_filas(data_dict):
            if proceso_cod:
                proc_data = list(data_dict.values())[0] if data_dict else None
                if proc_data:
                ...
                for proc_cod_val, proc_data in sorted(data_dict.items()):
                    for art_cod, art_data in sorted(proc_data["articulos"].items()):
                        for mov in art_data["movimientos"]:
                            ws.append([art_cod, art_data["nombre"], mov["fecha"], mov["ot"], mov["tipo"], mov["cantidad"], mov["um"], mov["encargado"]])
    ...
    def _generar_pdf(self, request: HttpRequest) -> HttpResponse:
        ...
        def build_elements():
            ...
            elems = []
            elems.append(Paragraph("Informe de Consumo por Proceso", title_style))
