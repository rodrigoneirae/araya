...
class IndexInformePEEncargadoView(LoginRequiredMixin, TemplateView):
    template_name = 'modulos/produccion/informes/pe_encargado.html'
    ...
    def _get_datos(self, fecha_inicio: str, fecha_fin: str, encargado_cod: str | None):
        ...
        if encargado_cod:
            ...
            qs = qs.exclude(codencargado__isnull=True).exclude(codencargado=0)
        ...
        for m in qs.iterator():
            ...
            if enc_cod not in data:
                data[enc_cod] = {"nombre": enc_nom, "articulos": {}}
            ...
            proceso_nom = procesos_map.get(proceso_cod, "")

            if cod_art not in data[enc_cod]["articulos"]:
                data[enc_cod]["articulos"][cod_art] = {"nombre": nom_art, "um": um_art, "movimientos": []}
    ...
    def _generar_excel(self, request: HttpRequest) -> HttpResponse:
        ...
        wb = Workbook()
        ws = wb.active
        ws.title = "Producción por Encargado"
        ...
        filename = "produccion_encargado.xlsx"
    ...
    def _generar_pdf(self, request: HttpRequest) -> HttpResponse:
        ...
        def build_elements():
            ...
            elems = []
            elems.append(Paragraph("Informe de Producción por Encargado", title_style))
            enc_nombre = "Todos los Encargados" if not encargado_cod else (list(data.values())[0]['nombre'] if data else '')
            elems.append(Paragraph(f"<b>Encargado:</b> {enc_nombre}", subtitle_style))
            if fecha_inf:
            ...
            def add_subtotal():
                nonlocal enc_subtotal
                if enc_subtotal > 0:
                    table_data.append([
                        Paragraph("", center_style),
                        Paragraph("<b>Total Encargado</b>", bold_style),
                        Paragraph("", center_style),
                        Paragraph("", center_style),
            ...
            if encargado_cod:
                ...
                for enc_cod, enc_data in sorted(data.items()):
                    current_enc = enc_data["nombre"]
                    enc_subtotal = 0
                    
                    table_data.append([
                        Paragraph("", center_style),
                        Paragraph(f"<b>=== {current_enc} ===</b>", bold_style),
                        Paragraph("", center_style),
                        Paragraph("", center_style),
                        Paragraph("", cell_style),
            ...
            for i in range(1, len(table_data)):
                ...
                if "===" in row_text or "Total Encargado" in row_text or "Total General" in row_text:
                    style_cmds.extend([
        ...
        filename = "produccion_encargado.pdf"
