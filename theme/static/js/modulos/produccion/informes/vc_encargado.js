...
function _fetchInforme(action, filename) {
    ...
    if (!encargado) {
        Toastify({ text: "Debe seleccionar un encargado", duration: 3000, gravity: "top", position: "right", style: { background: "#ef4444" } }).showToast();
        return;
    }
...
function generarInforme(formato) {
    if (formato === 'pdf') {
        _fetchInforme('generar_pdf', 'consumo_encargado.pdf');
    } else {
        _fetchInforme('generar_excel', 'consumo_encargado.xlsx');
    }
}
